...
![Kuusi kertaa kuusi -ruudukko, jonka punaisella väritetyt ruudut muodostavat porrasmaisen reitin vasemmasta alanurkasta oikeaan ylänurkkaan edeten vain oikealle ja ylöspäin.](assets/01-figure-01.svg)
...
![Sama ruudukko, jonka vasemman alanurkan lähimpiin ruutuihin on kirjoitettu reittien määrät: alarivillä ja vasemmassa reunassa lukee 1 ja lähimmissä sisäruuduissa kasvavat luvut 2, 3, 4 ja 6.](assets/01-figure-02.svg)
...
![Täysi kuusi kertaa kuusi -ruudukko, jonka jokaiseen ruutuun on merkitty reittien määrä. Luvut kasvavat reunojen ykkösistä sisäruutuja kohti ja oikeassa ylänurkassa lukee 252.](assets/01-figure-03.svg)
...
**Kommentti.** Jos siis $T(x, y)$ kuvaa ruutuun $(x, y)$ päättyvien reittien määrää, niin ratkaisun idean nojalla

$$T(x, y) = T(x-1, y) + T(x, y-1).$$

 Tätä ideaa kutsutaan **rekursioksi**: saamme laskettua vastauksen isompaan tapaukseen pienempien tapauksien avulla. Rekursio on tärkeä esimerkki induktiivisesta päättelystä.

## 1.3 Esimerkki 2: Summa

**

**Tehtävä 1.2** Osoita, että
...
Toisin sanoen meitä pyydetään todistamaan suora kaava ensimmäisen $n$ luvun summalle. Esimerkiksi

$$1 + 2 + 3 + \ldots + 100 = \frac{100 \cdot 101}{2} = 50 \cdot 101 = 5050.$$

Huomataan, että väite pätee ainakin pienillä arvoilla:
...
Mitä yhtälön [1.1](01 Induktiivinen päättely.md#eq-summa) vasemmalle ja oikealle puolelle tapahtuu, kun lukua $n$ kasvatetaan yhdellä?

**Vastaus.** Sekä vasen että oikea puoli kasvaa $n+1$ verran.

**Vastauksen perustelu.** Vasen puoli tietysti kasvaa $n+1$ verran: vasen puoli pysyy muuten samana, mutta summaan ilmestyy yksi uusi termi, nimittäin $n+1$. Oikean puolen muutosta ei ole helppo nähdä suoraan, vaan tätä varten pitää laskea. Muutos on uusi arvo miinus vanha arvo eli

$$\frac{(n+1)((n+1) + 1)}{2} - \frac{n(n+1)}{2}.$$

 Tämän saa helpoiten sievennettyä ottamalla $n+1$:n yhteiseksi tekijäksi:

$$\frac{(n+1)((n+1) + 1)}{2} - \frac{n(n+1)}{2} = (n+1)\left(\frac{n+2}{2} - \frac{n}{2}\right) = (n+1) \cdot 1 = n+1.$$

 Muutos on siis $n+1$, niin kuin pitikin.

**Mitä tästä hyötyy?** Olemme edellä tarkastaneet, että yhtälö [1.1](01 Induktiivinen päättely.md#eq-summa) pätee, kun $n = 1, 2, 3, 4$. Lisäksi olemme tarkastaneet, että jos lukua $n$ kasvattaa yhdellä, niin vasen ja oikea puoli muuttuvat saman verran, eli ne ovat edelleen yhtä suuria. Tästä seuraa, että yhtälö pätee myös suuremmilla $n$:n arvoilla.
...
![Kolme suoraa, jotka leikkaavat toisiaan lähellä keskustaa ja jakavat tason muutamaan lohkomaiseen alueeseen. Alueita ei ole vielä väritetty.](assets/01-figure-04.svg)
...
![Samat kolme suoraa neliömäisen alueen sisällä, ja niiden rajaamat alueet on väritetty harmaalla ja valkoisella niin, etteivät mitkään kaksi vierekkäistä aluetta ole samanvärisiä.](assets/01-figure-05.svg)

Tutkitaan sitten tilannetta, jossa suoria on yksi enemmän.

![Edelliseen kuvioon on lisätty uusi suora punaisella. Vanha harmaa-valkoinen väritys on ennallaan, joten punaisen suoran eri puolilla on nyt vierekkäisiä samanvärisiä alueita.](assets/01-figure-06.svg)
...
![Punaisen suoran yläpuolella olevien alueiden värejä on alettu vaihtaa päinvastaisiksi, mutta osa yläpuolen alueista on vielä vaihtamatta.](assets/01-figure-07.svg)

Väritystä pitää tietysti muuttaa vielä muidenkin uuden suoran yläpuolella olevien alueiden kohdalla:

![Kaikki punaisen suoran yläpuolella olevat alueet on nyt vaihdettu päinvastaiseen väriin, ja harmaa-valkoinen väritys on jälleen kelvollinen, kun mitkään kaksi vierekkäistä aluetta eivät ole samanvärisiä.](assets/01-figure-08.svg)
...
Määrät $f(n)$ ja $g(n)$ riippuvat toisistaan seuraavilla tavoilla:

$$f(n) = g(n-1) \qquad(1.2)$$

 ja

$$g(n) = 2f(n-1) + 2g(n-1). \qquad(1.3)$$

Perustelut: Jokainen $a$-kirjaimeen päättyvä $n$-kirjaiminen jono saadaan ottamalla jokin $n-1$-merkkinen jono, joka ei pääty $a$:han, ja lisäämällä loppuun $a$-kirjain. Yhtälö [1.2](01 Induktiivinen päättely.md#eq-ind-f) seuraa tästä.

Jokainen $n$-kirjaiminen jono, joka ei pääty $a$-kirjaimeen saadaan ottamalla mikä vain $n-1$-kirjaiminen jono ja lisäämällä sen loppuun $b$- tai $c$-kirjain. Tästä saadaan yhtälö [1.3](01 Induktiivinen päättely.md#eq-ind-g).

Alkuarvot saadaan helposti: $f(1) = 1$ ja $g(1) = 2$. Käyttämällä rekursioyhtälöitä [1.2](01 Induktiivinen päättely.md#eq-ind-f) ja [1.3](01 Induktiivinen päättely.md#eq-ind-g) voimme nyt laskea luvut $f(2)$ ja $g(2)$, sitten luvut $f(3)$ ja $g(3)$ ja niin edelleen. Alla on taulukko näistä arvoista.
...
![Nelikulmio, johon on piirretty punaisella sen kaksi lävistäjää.](assets/01-figure-09.svg)

![Viisikulmio, johon on piirretty punaisella sen viisi lävistäjää.](assets/01-figure-10.svg)
